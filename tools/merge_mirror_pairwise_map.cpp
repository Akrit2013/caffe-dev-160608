// This program is used in PROJ1603, it can merge the orginal pairwise map lmdb
// and the mirror pairwise map into a new pairwise map, which use the lower trangle
// store the original pairwise map ( H > W ), and use the upper trangle ( H < W )
// to store the mirror pairwise map.
//
// The input:
//  1. The lmdb containing the orginal pairwise map.
//  2. The lmdb containing the mirror pairwise map
// The output:
//  1. The packed pairwise map
//
// Usage:
//   merge_mirror_pairwise_map [FLAGS] original_pairwise_lmdb mirror_pairwise_lmdb out_lmdb
...
and the mirror pairwise map.\n"
        "Usage:\n"
        "    merge_mirror_pairwise_map [FLAGS] PAIRWISE_LMDB MIRROR_PAIRWISE_LMDB OUT_MERGED_LMDB");
  gflags::ParseCommandLineFlags(&argc, &argv, true);

  if (argc < 4) {
    gflags::ShowUsageWithFlagsRestrict(argv[0], "merge_mirror_pairwise_map");
    return 1;
  }
  ...
  while (cursor_mirr->valid() && cursor_pair->valid()){
	  ...
	  blob_out.Reshape(1, 1, datum_pair.height(), datum_pair.width());
	  ...
	  for (int n = 0; n < blob_out.num(); n++){
		  for (int c = 0; c < blob_out.channels(); c++){
			  for (int h = 0; h < blob_out.height(); h++){
				  for (int w = 0; w < h; w++){
					  out_data[blob_out.offset(n, c, h, w)] = pair_data[blob_pair.offset(n, c, h, w)];
					  out_data[blob_out.offset(n, c, w, h)] = mirr_data[blob_pair.offset(n, c, h, w)];
				  }
			  }
		  }
	  }
