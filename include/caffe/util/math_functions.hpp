...
template <typename Dtype>
void caffe_cpu_symm(const CBLAS_SIDE side,
...
// Added by YanHan
// Matrix product vector of symmetric matrix
template <typename Dtype>
void caffe_cpu_symv(const int M,
    const Dtype alpha, const Dtype* A, const Dtype* x, const Dtype beta,
    Dtype* y);
...
// Added by YanHan
// Return the max absolute value of the memory, the incx is the increase of the memory
// normally can be set to 1
// the size of the a is at least (1+(n-1)*abs(incx)).
template <typename Dtype>
Dtype caffe_amax(const int n, const Dtype* a, int incx=1); 

template <typename Dtype>
Dtype caffe_gpu_amax(const int n, const Dtype* a, int incx=1); 

// Added by YanHan
// Calc the determint of the matrix
template <typename Dtype>
Dtype caffe_cpu_det(const int n, const Dtype* m);
// Calc the inv of the matrix
// NOTE:
// Normally, the gpu version of calc matrix inv is used  to calc a batch of small
// matrixs inverse (such as 32x32), if the matrix is too big, it will considerably
// slower than the MKL or openblas
template <typename Dtype>
void caffe_gpu_inv(const int n, const int batch_size, const Dtype* m, Dtype* m_inv);
// Calc the inv of the matrix, which is much faster than the previous function
// only can be used when the input matrix is a symmetric positive definite matrix  
template <typename Dtype>
void caffe_cpu_inv_sympd(const int n, const Dtype* m, Dtype* m_inv);

template <typename Dtype>
void caffe_cpu_inv(const int n, const Dtype* m, Dtype* m_inv);
...
// Added by YanHan
// Reture the sum instead of the absolute sum
template <typename Dtype>
