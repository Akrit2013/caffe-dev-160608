...
#include "caffe/layers/crf2_loss_layer.hpp"
...
template <typename Dtype>
void Crf2LossLayer<Dtype>::Calc_A_gpu(void){
	// A = I + D -R
	// Set A to 0
...
template <typename Dtype>
void Crf2LossLayer<Dtype>::Calc_R_gpu(const Blob<Dtype>* bottom){
	...
	for (int n = 0; n < bottom->num(); n++){
		...
		for (int c = 0; c < bottom->channels(); c++){
			const Dtype* bottom_data_nc = bottom_data + bottom->offset(n, c);
			caffe_gpu_axpy(dim, -gamma_*weight_data[c], bottom_data_nc, r_data_n);
		}
	}
	// Calc the exp
	...
	// Mult the alpha
	if (alpha_ != 1){
		caffe_gpu_scal(count, alpha_, r_data);
	}

}

template <typename Dtype>
void Crf2LossLayer<Dtype>::Euclidean_loss_gpu(const Blob<Dtype>* gt, Blob<Dtype>* top){
...
template <typename Dtype>
void Crf2LossLayer<Dtype>::Inference_gpu(const Blob<Dtype>* Z){
...
template <typename Dtype>
void Crf2LossLayer<Dtype>::Calc_J_gpu(const Blob<Dtype>* bottom){
	const Dtype* bottom_data = bottom->gpu_data();
	const Dtype* r_data = R_.gpu_data();
...
template <typename Dtype>
void Crf2LossLayer<Dtype>::Pairwise_BP_gpu(const Blob<Dtype>* gt){
	const Dtype* a_data = A_inv_.gpu_data();
	...
	// Apply the beta
	this->blobs_[0]->scale_diff(beta_ / Dtype(num) / Dtype(dat_channels));
}

template <typename Dtype>
void Crf2LossLayer<Dtype>::Forward_gpu(const vector<Blob<Dtype>*>& bottom,
...
// ---- DEBUG -----
  ...
  if (this->phase_ == TRAIN && disp_w_ != 0 && counter_++ % disp_w_ == 0){
	  ...
	  LOG(INFO)<<"Weight: "<<weight[0]<<","<<weight[1]<<","<<weight[2]<<","<<weight[3]<<","<<weight[4]<<","<<weight[5]<<","<<weight[6]<<","<<weight[7];
  }
...
template <typename Dtype>
void Crf2LossLayer<Dtype>::Backward_gpu(const vector<Blob<Dtype>*>& top,
...
template <typename Dtype>
void Crf2LossLayer<Dtype>::Calc_A_inv_gpu(void){
	const int num = A_.num();
...
template void Crf2LossLayer<float>::Calc_A_gpu(void);
template void Crf2LossLayer<double>::Calc_A_gpu(void);

template void Crf2LossLayer<float>::Calc_R_gpu(const Blob<float>* bottom);
template void Crf2LossLayer<double>::Calc_R_gpu(const Blob<double>* bottom);

template void Crf2LossLayer<float>::Euclidean_loss_gpu(const Blob<float>* gt, Blob<float>* top);
template void Crf2LossLayer<double>::Euclidean_loss_gpu(const Blob<double>* gt, Blob<double>* top);

template void Crf2LossLayer<float>::Inference_gpu(const Blob<float>* Z);
template void Crf2LossLayer<double>::Inference_gpu(const Blob<double>* Z);

template void Crf2LossLayer<float>::Calc_J_gpu(const Blob<float>* bottom);
template void Crf2LossLayer<double>::Calc_J_gpu(const Blob<double>* bottom);

template void Crf2LossLayer<float>::Pairwise_BP_gpu(const Blob<float>* gt);
template void Crf2LossLayer<double>::Pairwise_BP_gpu(const Blob<double>* gt);

template void Crf2LossLayer<float>::Calc_A_inv_gpu(void);
template void Crf2LossLayer<double>::Calc_A_inv_gpu(void);

INSTANTIATE_LAYER_GPU_FUNCS(Crf2LossLayer);
