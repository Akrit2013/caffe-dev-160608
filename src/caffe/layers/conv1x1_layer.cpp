...
void Conv1x1Layer<Dtype>::compute_output_shape() {
...
template <typename Dtype>
void Conv1x1Layer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
...
template <typename Dtype>
void Conv1x1Layer<Dtype>::normalize_param(void){
	...
	for(int n = 0; n < num; n++){
		for(int h = 0; h < height; h++){
			for(int w = 0; w < width; w++){
				...
				if (this->normalize_){
					Dtype rate = this->norm_val_ / c_sum;
					for(int c = 0; c < channels; c++){
						int index = ((n*channels+c)*height+h)*width+w;
...
template <typename Dtype>
void Conv1x1Layer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
...
#ifdef CPU_ONLY
STUB_GPU(Conv1x1Layer);
#endif

INSTANTIATE_CLASS(Conv1x1Layer);
