...
__global__ void Backward_gpu_kernel(
	...
	CUDA_KERNEL_LOOP(index, nthreads){
		...
		for(int w = 0; w < width; w++){
			bottom_diff[data_offset+w] = sign * (w1 * diff_data[data_offset+w] - w2);
		}
	}
}

template <typename Dtype>
void ScaleInvariantBerhuLossLayer<Dtype>::Forward_gpu(const vector<Blob<Dtype>*>& bottom,
...
template <typename Dtype>
void ScaleInvariantBerhuLossLayer<Dtype>::Backward_gpu(const vector<Blob<Dtype>*>& top,
...
INSTANTIATE_LAYER_GPU_FUNCS(ScaleInvariantBerhuLossLayer);
