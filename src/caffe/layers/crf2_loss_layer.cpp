...
void Crf2LossLayer<Dtype>::LayerSetUp(
      const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top){
	LossLayer<Dtype>::LayerSetUp(bottom, top);
	// Parse the param
	Crf2LossParameter crf_param = this->layer_param_.crf2_loss_param();
	...
	if (crf_param.has_beta()){
		beta_ = crf_param.beta();
	}else{
		beta_ = 0.01;
	}

	if (crf_param.has_gamma()){
		gamma_ = crf_param.gamma();
	}else{
		gamma_ = 1;
	}

	if (crf_param.has_partition()){
		partition_ = crf_param.partition();
	}else{
		partition_ = true;
	}
	counter_ = 1;
	...
	if (crf_param.has_positive()){
		positive_ = crf_param.positive();
	}else{
		positive_ = true;
	}
	if (crf_param.has_norm_val()){
		norm_val_ = crf_param.norm_val();
	}else{
		norm_val_ = 1;
	}
	...
	weight_shape[0] = 1;
	weight_shape[1] = bottom[2]->channels();
	weight_shape[2] = 1;
	weight_shape[3] = 1;
	this->blobs_[0].reset(new Blob<Dtype>(weight_shape));
    shared_ptr<Filler<Dtype> > weight_filler(GetFiller<Dtype>(
        this->layer_param_.crf2_loss_param().weight_filler()));
...
template <typename Dtype>
void Crf2LossLayer<Dtype>::Reshape(
  const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top) {
...
template <typename Dtype>
void Crf2LossLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
...
template <typename Dtype>
void Crf2LossLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
...
template <typename Dtype>
void Crf2LossLayer<Dtype>::Calc_R_cpu(const Blob<Dtype>* bottom){
	...
	for (int n = 0; n < bottom->num(); n++){
		...
		for (int c = 0; c < bottom->channels(); c++){
			...
			caffe_axpy(dim, -gamma_*weight_data[c], bottom_data_nc, r_data_n);
		}
	}
	// Calc the exp
	...
	// Mult with the alpha
	if (alpha_ != 1){
		caffe_scal(count, alpha_, r_data);
	}
}

template <typename Dtype>
void Crf2LossLayer<Dtype>::Calc_A_cpu(void){
	// A = I + D -R
	// Set A to 0
...
template <typename Dtype>
void Crf2LossLayer<Dtype>::Inference_cpu(const Blob<Dtype>* Z){
...
template <typename Dtype>
void Crf2LossLayer<Dtype>::Pairwise_BP_cpu(const Blob<Dtype>* gt){
	...
	// Apply the beta_ and the channel number
	this->blobs_[0]->scale_diff(beta_ / Dtype(num) / Dtype(dat_channels));
}

template <typename Dtype>
void Crf2LossLayer<Dtype>::Calc_J_cpu(const Blob<Dtype>* bottom){
	const Dtype* bottom_data = bottom->cpu_data();
	const Dtype* r_data = R_.cpu_data();
	...
	for (int n = 0; n < J_.num(); n++){
		for (int c = 0; c < J_.channels(); c++){
			for (int h = 0; h < J_.height(); h++){
				for (int w = 0; w < J_.width(); w++){
					const int index = J_.offset(n, c, h, w);
					const int r_index = R_.offset(n, 0, h, w);
					j_data[index] = gamma_*bottom_data[index]*r_data[r_index];
				}
			}
...
template <typename Dtype>
void Crf2LossLayer<Dtype>::Normalize_weight_cpu(void){
...
template <typename Dtype>
void Crf2LossLayer<Dtype>::Calc_A_inv_cpu(void){
...
template <typename Dtype>
void Crf2LossLayer<Dtype>::Euclidean_loss_cpu(const Blob<Dtype>* gt, Blob<Dtype>* top){
...
#ifdef CPU_ONLY
STUB_GPU(Crf2LossLayer);
#endif

INSTANTIATE_CLASS(Crf2LossLayer);
REGISTER_LAYER_CLASS(Crf2Loss);
