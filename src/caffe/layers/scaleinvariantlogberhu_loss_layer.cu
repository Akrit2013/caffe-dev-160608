...
template <typename Dtype>
__global__ void Berhu_gpu_kernel(
		const int nthreads,
		Dtype* data_diff,
		const int num,
		const int channels,
	...
	CUDA_KERNEL_LOOP(index, nthreads){
		...
		// Iter the width and channels
		for (int w = 0; w < width; w++){
			const int idx = data_offset + w;
			Dtype diffval = data_diff[idx];

			// alter the diff value
			if (diffval > c_boundary && diffval < C){
				// L1
				data_diff[idx] = C;
			}else if(diffval < -c_boundary && -diffval < C){
				data_diff[idx] = -C;
			}
...
template <typename Dtype>
void ScaleInvariantLogBerhuLossLayer<Dtype>::Forward_gpu(const vector<Blob<Dtype>*>& bottom,
  ...
  const int num_kernels2 = num * channels * height;
  // Re-check the diff_ again to apply the C and H
  Berhu_gpu_kernel<Dtype><<<CAFFE_GET_BLOCKS(num_kernels2), CAFFE_CUDA_NUM_THREADS>>>(
		  num_kernels2,
		  data_diff,
		  num,
		  channels,
...
template <typename Dtype>
void ScaleInvariantLogBerhuLossLayer<Dtype>::Backward_gpu(const vector<Blob<Dtype>*>& top,
...
INSTANTIATE_LAYER_GPU_FUNCS(ScaleInvariantLogBerhuLossLayer);
